...
def build_model(dim, learn_rate, lmbda, drop, FL, init, n_filters):
    ...
    conv1 = Convolution2D(n_filters, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(img_input)
    conv1 = BatchNormalization()(conv1)
    conv1 = Convolution2D(n_filters, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(conv1)
    conv1 = BatchNormalization()(conv1)
    conv1 = Add()([conv1,img_input])
    pool1 = MaxPooling2D((2, 2), strides=(2, 2))(conv1)


    conv2in = Convolution2D(n_filters * 2, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(pool1)
    conv2in = BatchNormalization()(conv2in)
    conv2 = Convolution2D(n_filters * 2, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(conv2in)
    conv2 = BatchNormalization()(conv2)
    conv2 = Add()([conv2,conv2in])
    pool2 = MaxPooling2D((2, 2), strides=(2, 2))(conv2)


    conv3in = Convolution2D(n_filters * 4, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(pool2)
    conv3in = BatchNormalization()(conv3in)
    conv3 = Convolution2D(n_filters * 4, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(conv3in)
    conv3 = BatchNormalization()(conv3)
    conv3 = Add()([conv3,conv3in])
    pool3 = MaxPooling2D((2, 2), strides=(2, 2),)(conv3)


    conv4in = Convolution2D(n_filters * 8, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(pool3)
    conv4in = BatchNormalization()(conv4in)
    conv4 = Convolution2D(n_filters * 8, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(conv4in)
    conv4 = BatchNormalization()(conv4)
    conv4 = Add()([conv4,conv4in])
    conv4 = Dropout(drop)(conv4)
    pool4 = MaxPooling2D((2, 2), strides=(2, 2),)(conv4)


    conv5in= Convolution2D(n_filters * 16, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(pool4)
    conv5in = BatchNormalization()(conv5in)
    conv5 = Convolution2D(n_filters * 16, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(conv5in)
    conv5 = BatchNormalization()(conv5)
    conv5 = Add()([conv5,conv5in])
    conv5 = Dropout(drop)(conv5)
    #pool5 = MaxPooling2D((2,2),strides=(2,2),)(conv5)


    u4 = UpSampling2D((2, 2))(conv5)
    u4 = Convolution2D(n_filters * 8, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u4)
    u4 = BatchNormalization()(u4)
    u4c = merge((conv4, u4), mode='concat', concat_axis=3)
    u4c =BatchNormalization()(u4c)
    u4 = Convolution2D(n_filters * 8, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u4c)
    u4in = BatchNormalization()(u4)
    u4 = Convolution2D(n_filters * 8, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u4in)
    u4 = BatchNormalization()(u4)
    u4 = Add()([u4,u4in])


    u3 = UpSampling2D((2, 2))(u4)
    u3 = Convolution2D(n_filters * 4, FL, FL, activation='relu', init=init,
                      W_regularizer=l2(lmbda), border_mode='same')(u3)
    u3 = BatchNormalization()(u3)
    u3c = merge((conv3, u3), mode='concat', concat_axis=3)
    u3c = BatchNormalization()(u3c)
    u3 = Convolution2D(n_filters * 4, FL, FL, activation='relu', init=init,
                      W_regularizer=l2(lmbda), border_mode='same')(u3c)
    u3in = BatchNormalization()(u3)
    u3 = Convolution2D(n_filters * 4, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u3in)
    u3 = BatchNormalization()(u3)
    u3 = Add()([u3,u3in])


    u2 = UpSampling2D((2, 2))(u3)
    u2 = Convolution2D(n_filters * 2, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u2)
    u2 = BatchNormalization()(u2)
    u2c = merge((conv2, u2), mode='concat', concat_axis=3)
    u2c = BatchNormalization()(u2c)
    u2 = Convolution2D(n_filters * 2, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u2c)
    u2in = BatchNormalization()(u2)
    u2 = Convolution2D(n_filters * 2, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u2in)
    u2 = BatchNormalization()(u2)
    u2 = Add()([u2, u2in])


    u = UpSampling2D((2, 2))(u2)
    u = Convolution2D(n_filters, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(u)
    u = BatchNormalization()(u)
    uc = merge((conv1, u), mode='concat', concat_axis=3)
    uc = BatchNormalization()(uc)
    u = Convolution2D(n_filters, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(uc)
    uin = BatchNormalization()(u)
    u = Convolution2D(n_filters, FL, FL, activation='relu', init=init,
                       W_regularizer=l2(lmbda), border_mode='same')(uin)
    u = BatchNormalization()(u)
    u = Add()([u, uin])
